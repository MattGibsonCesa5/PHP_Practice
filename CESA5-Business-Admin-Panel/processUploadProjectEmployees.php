        ...
        if (isset($_SESSION["role"]) && ($_SESSION["role"] == 1 || $_SESSION["role"] == 2))
        {
            ...
            ?>
                ...
                <div class="row text-center">
                    ...
                    <?php
                        if (isset($_POST["ProjEmp-Up-period_id"]))
                        {
                            ...
                            if (verifyPeriod($conn, $period_id)) // verify the period selected exists
                            {
                                if (isset($_FILES["fileToUpload"])) 
                                {
                                    ...
                                    if (isset($file) && (isset($file_type) && $file_type == "text/csv"))
                                    {
                                        ...
                                        while ($data = fgetcsv($handle, 1000, ",", '"'))
                                        {
                                            if (isset($data[0]) && $data[0] != "Employee ID") // skip the first row by looking at cell data
                                            {
                                                ...
                                                if ($employee != null && $project != null && $days != null && $obj != null && $func != null)
                                                {
                                                    ...
                                                    { 
                                                        $errors++;
                                                        echo "<span class=\"log-fail\">Failed</span> to add/update the employee with ID $employee to the Project $project budget. The days in project must be a valid number greater than 0!<br>"; 
                                                    }
                                                }
                                                else 
                                                { 
                                        ...
                                        $message = "Successfully uploaded $total_successes employees to project(s) ($inserted inserts; $updated updates). ";
                                        if ($errors > 0) { $message .= "Failed to upload $errors employees to project(s). "; }
                                        $log = mysqli_prepare($conn, "INSERT INTO log (user_id, message) VALUES (?, ?)");
                                        mysqli_stmt_bind_param($log, "is", $_SESSION["id"], $message);
                                        mysqli_stmt_execute($log);
                                    }
                                    else { echo "<i class='fa-solid fa-triangle-exclamation'></i> ERROR! You must select a .csv file to upload.<br>"; }
                                }   
                                else { echo "<i class='fa-solid fa-triangle-exclamation'></i> ERROR! No upload file was found. Please select a file to upload and try again.<br>"; }
                            }
