                        ...
                        if (isset($_POST["ProjExp-BulkUp-period_id"]))
                        {
                            ...
                            if (verifyPeriod($conn, $period_id)) // verify the period selected exists
                            {
                                if (isset($_FILES["files"]))
                                {
                                            ...
                                            if (strlen($_FILES["files"]["name"][$i]) > 1)
                                            {
                                                ...
                                                while ($data = fgetcsv($handle, 1000, ",", '"'))
                                                {
                                                    if (isset($data[0]) && ($data[0] != "WUFAR Codes" && $data[0] != "Fund")) // skip the first two rows by looking at cell data
                                                    {
                                                        ...
                                                        if (isset($data[3]) && $data[3] <> "") { $func = clean_data($data[3]); } else { $desc = null; } 
                                    ...
                                    $message = "Successfully bulk uploaded $total_successes project expenses ($files_counter files). ";
                                    if ($errors > 0) { $message .= "Failed to upload $errors project expenses. "; }
