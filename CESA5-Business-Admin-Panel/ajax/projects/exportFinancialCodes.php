<?php 
    ...
    if (isset($_SESSION["status"]) && $_SESSION["status"] == 1)
    {
        if (isset($_SESSION["role"]) && $_SESSION["role"] == 1)
        {
            ...
            if (isset($_POST["export-period"]) && $_POST["export-period"] <> "") { $period = $_POST["export-period"]; } else { $period = null; }
                ...
                if ($period_id = getPeriodID($conn, $period)) // verify the period exists; if it exists, store the period ID
                {
                    ...
                    if (mysqli_stmt_execute($getProjects))
                    {
                        ...
                        if (mysqli_num_rows($getProjectsResults) > 0) // active projects found; continue
                        {
                            ...
                            $exportFile = "$period - Consolidated Codes.csv";
                                ...
                                if (mysqli_num_rows($getConsolidatedCodesResults) > 0) // codes found
                                {
                                    while ($code_pair = mysqli_fetch_array($getConsolidatedCodesResults))
                                    {
                                        ...
                                        if ($consolidated_cost > 0)
                                        {
                                            ...
                                            $amount = round($consolidated_cost, 2);

                                            fputcsv($output, [$account_code, $amount]);
                                        }
                                    }
                                }
                            }
