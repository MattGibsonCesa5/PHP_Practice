    ...
    if (isset($_SESSION["status"]) && $_SESSION["status"] == 1)
    {
        if (isset($_SESSION["role"]) && $_SESSION["role"] == 1)
        {
            ...
            if ($period != null && $period_id = getPeriodID($conn, $period))
            {
                ...
                $getProjects = mysqli_prepare($conn, "SELECT DISTINCT p.code, p.fund_code, p.function_code FROM projects p 
                ...
                if (mysqli_stmt_execute($getProjects))
                {
                    ...
                    if (mysqli_num_rows($getProjectsResults) > 0) // active projects found; continue
                    {
                        while ($project = mysqli_fetch_array($getProjectsResults))
                        {
                            // initialize array to store all codes for the project
                            $codes = [];

                            // initialize variables
                            $project_fund = $project_func = null;
                            
                            // store project details locally
                            $code = $project["code"];
                            if (isset($project["fund_code"]) && $project["fund_code"] <> "") { $project_fund = $project["fund_code"]; }
                            if (isset($project["function_code"]) && $project["function_code"] <> "") { $project_func = $project["function_code"]; }

                            // get the FTE days value
                            $FTE_Days = getFTEDays($conn);

                            // get all employee codes
                            $getProjEmps = mysqli_prepare($conn, "SELECT pe.object_code, pe.fund_code, pe.function_code, pe.project_days, ec.yearly_rate, ec.contract_days, health_insurance, dental_insurance, wrs_eligible FROM project_employees pe 
                            ...
                            if (mysqli_stmt_execute($getProjEmps))
                            {
                                ...
                                if (mysqli_num_rows($getProjEmpsResults) > 0) // project employees found
                                {
                                    while ($proj_emp = mysqli_fetch_array($getProjEmpsResults))
                                    {
                                        ...
                                        $fund = $proj_emp["fund_code"];
                                        $obj = $proj_emp["object_code"];
                                        $func = $proj_emp["function_code"];

                                        // if fund and func are not stored with the employee, assume the project's default
                                        if (!isset($fund)) { $fund = $project_fund; }
                                        if (!isset($func)) { $func = $project_func; }
                            ...
                            if (mysqli_stmt_execute($getProjExps))
                            {
                                ...
                                if (mysqli_num_rows($getProjExpsResults) > 0) // project expenses found
                                {
                                    while ($proj_exp = mysqli_fetch_array($getProjExpsResults)) 
                                    {
                                        ...
                                        // if exp fund not set, use project fund by default
                                        if (!isset($exp_fund) || $exp_fund == null) { $exp_fund = $project_fund; }

                                        // round the cost
                                        if (isset($cost) && $cost != null) { $cost = $cost; } else { $cost = 0.00; }
                            ...
                            for ($c = 0; $c < count($codes); $c++)
                            {
                                ...
                                // if the code is not set, set to null
                                if (!isset($fund)) { $fund = null; }
                        ...
                        $getConsolidatedCodes = mysqli_prepare($conn, "SELECT DISTINCT category, fund_code, location_code, object_code, function_code, project_code FROM codes_tmp 
                                                                        WHERE user_id=? AND user_hash=?
                                                                        ORDER BY project_code ASC, fund_code ASC, object_code ASC, function_code ASC, location_code ASC");
                        ...
                        if (mysqli_stmt_execute($getConsolidatedCodes))
                        {
                            ...
                            if (mysqli_num_rows($getConsolidatedCodesResults) > 0) // codes found
                            {
                                while ($code_pair = mysqli_fetch_array($getConsolidatedCodesResults))
                                {
                                    ...
                                    {
                                        if (!isset($fund) || $fund == null) { $fund = "<span class='missing-field'>Missing</span>"; }
                                        if (!isset($loc) || $loc == null) { $loc = "<span class='missing-field'>Missing</span>"; }
                                        if (!isset($obj) || $obj == null) { $obj = "<span class='missing-field'>Missing</span>"; }
                                        if (!isset($func) || $func == null) { $func = "<span class='missing-field'>Missing</span>"; }
                                        if (!isset($proj) || $proj == null) { $proj = "<span class='missing-field'>Missing</span>"; }

                                        // round the cost to 2 decimal places
                                        $amount = $consolidated_cost;
