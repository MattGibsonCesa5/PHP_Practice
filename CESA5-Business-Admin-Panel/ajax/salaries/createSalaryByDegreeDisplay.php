<?php 
    ...
    { 
        if (isset($_SESSION["role"]) && ($_SESSION["role"] == 1 || $_SESSION["role"] == 2))
        {
            ...
            $degrees = []; // initialize array to store degrees
            $getDegrees = mysqli_query($conn, "SELECT code, label FROM degrees ORDER BY code ASC");
            if (mysqli_num_rows($getDegrees) > 0) // degrees found
            {
                while ($degree = mysqli_fetch_array($getDegrees))
                {
                    $degrees[] = $degree["code"] . " - " . $degree["label"];
                }
            }

            ?>
                <!-- Degree x Experience Salary Breakdown Table -->
                <table class="report_table w-100" id="salaries-by_degree">
                    <thead>
                        <tr>
                            <th class="text-center p-1" rowspan="2">Degree</th>
                    ...
                    <tbody>
                        <?php
                            for ($d = 0; $d < count($degrees); $d++)
                            {
                                ?>
                                    <tr>
                                        <td><?php echo $degrees[$d]; ?></td>
                                        <?php 
                                            // get the average salary by years of experience for the degree
                                            for ($x = 0; $x <= 16; $x++)
                                            {
                                                if ($position <> "" && $area <> "")
                                                {
                                                    ...
                                                    if ($x != 16)
                                                    {
                                                        $getAvgSalary = mysqli_prepare($conn, "SELECT AVG(total_salary) AS avg_salary FROM dpi_employees WHERE assignment_position=? AND assignment_area=? AND contract_high_degree=? AND total_experience=? AND (work_type LIKE ? AND work_county LIKE ? AND work_level LIKE ?) AND total_salary>0");
                                                        mysqli_stmt_bind_param($getAvgSalary, "sssisss", $position, $area, $degrees[$d], $x, $work_type_filter, $work_county_filter, $work_level_filter);
                                                ...
                                                else
                                                {
                                                    ...
                                                    if ($x != 16)
                                                    {
                                                        $getAvgSalary = mysqli_prepare($conn, "SELECT AVG(total_salary) AS avg_salary FROM dpi_employees WHERE assignment_position LIKE ? AND assignment_area LIKE ? AND contract_high_degree=? AND total_experience=? AND work_type LIKE ? AND work_county LIKE ? AND work_level LIKE ? AND total_salary>0");
                                                        mysqli_stmt_bind_param($getAvgSalary, "sssisss", $position_filter, $area_filter, $degrees[$d], $x, $work_type_filter, $work_county_filter, $work_level_filter);
                                                        if (mysqli_stmt_execute($getAvgSalary))
                                                        {
                                                            $getAvgSalaryResult = mysqli_stmt_get_result($getAvgSalary);
                                                            if (mysqli_num_rows($getAvgSalaryResult) > 0)
