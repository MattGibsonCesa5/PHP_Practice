<?php 
    ...
    { 
        ...
        if (checkUserPermission($conn, "VIEW_SALARY_COMPARISON_STATE"))
        {
            ...
            ?>
                <!-- Gender x Experience Salary Breakdown Table -->
                <table class="report_table w-100" id="salaries-by_gender">
                    <thead>
                        <tr>
                            <th class="text-center p-1" rowspan="2">Gender</th>
                    ...
                    <tbody>
                        <tr>
                            <td>Male</td>
                            <?php 
                                // get the average salary by years of experience for the gender
                                for ($x = 0; $x <= 16; $x++)
                                {
                                    if ($position <> "" && $area <> "")
                                    {
                                        ...
                                        if ($x != 16)
                                        {
                                            $getAvgSalary = mysqli_prepare($conn, "SELECT AVG(total_salary) AS avg_salary FROM dpi_employees WHERE assignment_position=? AND assignment_area=? AND gender=1 AND total_experience=? AND (work_type LIKE ? AND work_county LIKE ? AND work_level LIKE ?) AND total_salary>0");
                                    ...
                                    else
                                    {
                                        ...
                                        if ($x != 16)
                                        {
                                            $getAvgSalary = mysqli_prepare($conn, "SELECT AVG(total_salary) AS avg_salary FROM dpi_employees WHERE assignment_position LIKE ? AND assignment_area LIKE ? AND gender=1 AND total_experience=? AND (work_type LIKE ? AND work_county LIKE ? AND work_level LIKE ?) AND total_salary>0");
                            ...
                            <?php 
                                ...
                                for ($x = 0; $x <= 16; $x++)
                                {
                                    if ($position <> "" && $area <> "")
                                    {
                                        ...
                                        if ($x != 16)
                                        {
                                            $getAvgSalary = mysqli_prepare($conn, "SELECT AVG(total_salary) AS avg_salary FROM dpi_employees WHERE assignment_position=? AND assignment_area=? AND gender=2 AND total_experience=? AND (work_type LIKE ? AND work_county LIKE ? AND work_level LIKE ?) AND total_salary>0");
                                            mysqli_stmt_bind_param($getAvgSalary, "ssisss", $position, $area, $x, $work_type_filter, $work_county_filter, $work_level_filter);
                                            if (mysqli_stmt_execute($getAvgSalary))
                                            {
                                                $getAvgSalaryResult = mysqli_stmt_get_result($getAvgSalary);
                                    ...
                                    else
                                    {
                                        ...
                                        if ($x != 16)
                                        {
                                            $getAvgSalary = mysqli_prepare($conn, "SELECT AVG(total_salary) AS avg_salary FROM dpi_employees WHERE assignment_position LIKE ? AND assignment_area LIKE ? AND gender=2 AND total_experience=? AND (work_type LIKE ? AND work_county LIKE ? AND work_level LIKE ?) AND total_salary>0");
                                            mysqli_stmt_bind_param($getAvgSalary, "ssisss", $position_filter, $area_filter, $x, $work_type_filter, $work_county_filter, $work_level_filter);
                                            if (mysqli_stmt_execute($getAvgSalary))
                                            {
                                                $getAvgSalaryResult = mysqli_stmt_get_result($getAvgSalary);
