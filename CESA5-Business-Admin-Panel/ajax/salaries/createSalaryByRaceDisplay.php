<?php 
    ...
    { 
        ...
        if (checkUserPermission($conn, "VIEW_SALARY_COMPARISON_STATE"))
        {
            ...
            // get a list of distinct races
            $races = []; // initialize array to store races
            $getDistinctRaces = mysqli_query($conn, "SELECT DISTINCT(race) FROM dpi_employees ORDER BY race ASC");
            if (mysqli_num_rows($getDistinctRaces) > 0) // races found
            {
                while ($race = mysqli_fetch_array($getDistinctRaces))
                {
                    $races[] = $race["race"];
                }
            }
            ...
            ?>
                <!-- Race x Experience Salary Breakdown Table -->
                <table class="report_table w-100" id="salaries-by_race">
                    <thead>
                        <tr>
                            <th class="text-center p-1" rowspan="2">Race</th>
                    ...
                    <tbody>
                        <?php
                            for ($r = 0; $r < count($races); $r++)
                            {
                                ?>
                                    <tr>
                                        <td><?php echo $races[$r]; ?></td>
                                        <?php 
                                            // get the average salary by years of experience for the race
                                            for ($x = 0; $x <= 16; $x++)
                                            {
                                                if ($position <> "" && $area <> "")
                                                {
                                                    ...
                                                    if ($x != 16)
                                                    {
                                                        $getAvgSalary = mysqli_prepare($conn, "SELECT AVG(total_salary) AS avg_salary FROM dpi_employees WHERE assignment_position=? AND assignment_area=? AND race=? AND total_experience=? AND (work_type LIKE ? AND work_county LIKE ? AND work_level LIKE ?) AND total_salary>0");
                                                        mysqli_stmt_bind_param($getAvgSalary, "sssisss", $position, $area, $races[$r], $x, $work_type_filter, $work_county_filter, $work_level_filter);
                                                ...
                                                else
                                                {
                                                    ...
                                                    if ($x != 16)
                                                    {
                                                        $getAvgSalary = mysqli_prepare($conn, "SELECT AVG(total_salary) AS avg_salary FROM dpi_employees WHERE assignment_position LIKE ? AND assignment_area LIKE ? AND race=? AND total_experience=? AND (work_type LIKE ? AND work_county LIKE ? AND work_level LIKE ?) AND total_salary>0");
                                                        mysqli_stmt_bind_param($getAvgSalary, "sssisss", $position_filter, $area_filter, $races[$r], $x, $work_type_filter, $work_county_filter, $work_level_filter);
                                                        if (mysqli_stmt_execute($getAvgSalary))
                                                        {
                                                            $getAvgSalaryResult = mysqli_stmt_get_result($getAvgSalary);
                                                            if (mysqli_num_rows($getAvgSalaryResult) > 0)
