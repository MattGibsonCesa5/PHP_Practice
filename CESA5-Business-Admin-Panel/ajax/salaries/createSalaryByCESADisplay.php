    ...
    { 
        ...
        if (checkUserPermission($conn, "VIEW_SALARY_COMPARISON_STATE"))
        {
            ...
            ?>
                ...
                <table class="report_table w-100" id="salaries-by_CESA">
                    <thead>
                        <tr>
                            <th class="text-center p-1" colspan="12">CESA</th>
                        </tr>
                    ...
                    <tbody>
                        <tr>
                            <?php 
                                // get the average salary by CESA
                                for ($c = 1; $c <= 12; $c++)
                                {
                                    if ($position <> "" && $area <> "")
                                    {
                                        $avg_salary = null;
                                        $getAvgSalary = mysqli_prepare($conn, "SELECT AVG(total_salary) AS avg_salary FROM dpi_employees WHERE assignment_position=? AND assignment_area=? AND work_cesa=? AND (work_type LIKE ? AND work_county LIKE ? AND work_level LIKE ?) AND total_salary>0");
                                        mysqli_stmt_bind_param($getAvgSalary, "ssisss", $position, $area, $c, $work_type_filter, $work_county_filter, $work_level_filter);
                                    ...
                                    {
                                        $avg_salary = null;
                                        $getAvgSalary = mysqli_prepare($conn, "SELECT AVG(total_salary) AS avg_salary FROM dpi_employees WHERE assignment_position LIKE ? AND assignment_area LIKE ? AND work_cesa=? AND work_type LIKE ? AND work_county LIKE ? AND work_level LIKE ? AND total_salary>0");
                                        mysqli_stmt_bind_param($getAvgSalary, "ssisss", $position_filter, $area_filter, $c, $work_type_filter, $work_county_filter, $work_level_filter);
                                        if (mysqli_stmt_execute($getAvgSalary))
                                        {
                                            $getAvgSalaryResult = mysqli_stmt_get_result($getAvgSalary);
                                            if (mysqli_num_rows($getAvgSalaryResult) > 0)
