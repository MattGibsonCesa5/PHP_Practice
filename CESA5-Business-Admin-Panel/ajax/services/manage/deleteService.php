        ...
        if (checkUserPermission($conn, "DELETE_SERVICES"))
        {
            ...
            if ($service_id != null && $service_id <> "")
            {
                ...
                if (mysqli_stmt_execute($checkService))
                {
                    ...
                    {
                        // delete the service
                        $deleteService = mysqli_prepare($conn, "DELETE FROM services WHERE id=?");
                        ...
                        if (mysqli_stmt_execute($deleteService)) // successfully deleted the service; delete other data associated with this service
                        {
                            ...
                            $deleteServicesProvided = mysqli_prepare($conn, "DELETE FROM services_provided WHERE service_id=?");
                            ...
                            // delete the costs we have associated to this service
                            $deleteServiceCosts = mysqli_prepare($conn, "DELETE FROM costs WHERE service_id=?");
                            mysqli_stmt_bind_param($deleteServiceCosts, "s", $service_id);
                            if (!mysqli_stmt_execute($deleteServiceCosts)) { echo "<span class=\"log-fail\">Failed</span> to delete the costs assoicated with this service.<br>"; }

                            // delete the quarterly costs we have associated to this service
                            $deleteServiceQuarterlyCosts = mysqli_prepare($conn, "DELETE FROM quarterly_costs WHERE service_id=?");
                            ...
                            $message = "Successfully deleted the service with ID $service_id.<br>";
                    ...
                    else { echo "<span class=\"log-fail\">Failed</span> to delete the service. The service you are trying to delete does not exists.<br>"; }
                }
                else { echo "<span class=\"log-fail\">Failed</span> to delete the service. An unknown error has occurred. Please try again later.<br>"; }
            }
