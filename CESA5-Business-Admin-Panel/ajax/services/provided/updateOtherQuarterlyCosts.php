                ...
                if ($invoice_id != null && is_numeric($invoice_id))
                {
                    if ($q1 != null && $q2 != null && $q3 != null && $q4 != null)
                    {
                        ...
                        if (is_numeric($q1) && is_numeric($q2) && is_numeric($q3) && is_numeric($q4)) // quarterly costs are numeric; continue
                        {
                            ...
                            $getTotalCost = mysqli_prepare($conn, "SELECT SUM(cost) AS total_cost FROM other_quarterly_costs WHERE other_invoice_id=?");
                            ...
                            if (mysqli_stmt_execute($getTotalCost))
                            {
                                ...
                                if (mysqli_num_rows($result) > 0)
                                {
                                    ...
                                    if (number_format($quarterly_cost_sum, 2, ".", "") == number_format($total_cost, 2, ".", "")) // quarterly costs equal the total cost; proceed with updates
                                    { 
                                        if (updateOtherQuarterlyCost($conn, $invoice_id, 1, $q1, $period_id))
                                        {
                                            if (updateOtherQuarterlyCost($conn, $invoice_id, 2, $q2, $period_id))
                                            {
                                                if (updateOtherQuarterlyCost($conn, $invoice_id, 3, $q3, $period_id))
                                                {
                                                    if (updateOtherQuarterlyCost($conn, $invoice_id, 4, $q4, $period_id))
                                                    {
                                                        // log quarterly costs update
                                                        $message = "Successfully updated quarterly costs for an \"Other Service\" with the invoice of ID $invoice_id.";
