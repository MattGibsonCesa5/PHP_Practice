    ...
    if (isset($_SESSION["status"]) && $_SESSION["status"] == 1)
    {
        ...
        if (checkUserPermission($conn, "VIEW_INVOICES_ALL") || checkUserPermission($conn, "VIEW_INVOICES_ASSIGNED"))
        {
            ...
            if ($invoice_id != null && $invoice_id <> "")
            {
                ...
                $getInvoice = mysqli_prepare($conn, "SELECT total_cost, quantity, description, date_provided FROM services_other_provided WHERE id=?");
                ...
                if (mysqli_stmt_execute($getInvoice));
                {
                    ...
                    if (mysqli_num_rows($getInvoiceResult) > 0) // invoice exists; continue
                    {
                        ...
                        // create the modal
                        ?>
                            <div class="modal fade" tabindex="-1" role="dialog" id="otherInvoiceDetailsModal" aria-labelledby="otherInvoiceDetailsModalLabel" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header primary-modal-header">
                                            <h5 class="modal-title primary-modal-title" id="otherInvoiceDetailsModalLabel">Invoice Details</h5>
