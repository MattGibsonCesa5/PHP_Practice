    ...
    if (isset($_SESSION["status"]) && $_SESSION["status"] == 1)
    {
        ...
        if (checkUserPermission($conn, "INVOICE_OTHER_SERVICES"))
        {
            ...
            if ($invoice_id != null && $invoice_id <> "")
            {
                ...
                $getServiceID = mysqli_prepare($conn, "SELECT service_id, customer_id FROM services_other_provided WHERE id=?");
                ...
                if (mysqli_stmt_execute($getServiceID))
                {
                    ...
                    if (mysqli_num_rows($getServiceIDResult) > 0) // invoice exists; continue
                    {
                        ...
                        $deleteInvoice = mysqli_prepare($conn, "DELETE FROM services_other_provided WHERE id=?");
                        ...
                        if (mysqli_stmt_execute($deleteInvoice)) // successfully deleted the invoice
                        {
                            ...
                            $deleteCosts = mysqli_prepare($conn, "DELETE FROM other_quarterly_costs WHERE other_invoice_id=?");
        ...
        else { echo "<span class=\"log-fail\">Failed</span> to delete the invoice. Your account does not have permission to manage invoices for other services!<br>"; }
