<?php 
    ...
    if (isset($_SESSION["status"]) && $_SESSION["status"] == 1)
    {
        ...
        if (checkUserPermission($conn, "VIEW_CASELOADS_ALL") || checkUserPermission($conn, "VIEW_CASELOADS_ASSIGNED"))
        {
            ...
            if ($caseload_id != null && verifyUserCaseload($conn, $caseload_id))
            {
                ...
                if ($period != null && $period_id = getPeriodID($conn, $period))
                {
                    ...
                    $getCases = mysqli_prepare($conn, "SELECT c.*, cl.employee_id, s.fname, s.lname, s.date_of_birth FROM cases c
                                                        ...
                                                        WHERE c.period_id=? AND c.caseload_id=? AND ((c.end_date<(SELECT p.caseload_term_end FROM periods p WHERE p.id=?) AND c.evaluation_method=1) OR (c.evaluation_method=2))");
                    ...
                    if (mysqli_stmt_execute($getCases))
                    {
                        ...
                        if (mysqli_num_rows($getCasesResults) > 0) // cases exist; continue
                        {
                            while ($case = mysqli_fetch_array($getCasesResults))
                            {
                                ...
                                $dismissal_reasoning_id = $case["dismissal_reasoning_id"];
                                ...
                                // build the term column
                                $daterange = "<b>Start:</b> ".$start_date."<br><b>End:</b> ".$end_date;

                                // if the student was dismissed, get the reasoning
                                $dismissal_reasoning = "";
                                if ($dismissed == 1)
                                {
                                    $dismissal_reasoning = getDismissalReasoning($conn, $dismissal_reasoning_id);
                                }
                                ...
                                $temp["dismissal_reasoning"] = $dismissal_reasoning;
                                $temp["end_notes"] = "";
                                $temp["assistant"] = $assistant_name;
                                $temp["actions"] = $actions;
                                $temp["status"] = $filter_status;
                                $temp["evaluation"] = $filter_evaluation_method;
