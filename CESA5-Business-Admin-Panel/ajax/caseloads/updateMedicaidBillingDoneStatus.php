    ...
    if (isset($_SESSION["status"]) && $_SESSION["status"] == 1)
    {
        ...
        if (checkUserPermission($conn, "VIEW_CASELOADS_ALL") || checkUserPermission($conn, "VIEW_CASELOADS_ASSIGNED"))
        {
            ...
            if ($case_id != null && verifyCase($conn, $case_id))
            {
                ...
                $updateStatus = mysqli_prepare($conn, "UPDATE cases SET medicaid_billing_done=? WHERE id=?");
                ...
                if (mysqli_stmt_execute($updateStatus)) 
                {
                    ...
                    $message = "Set the medicaid billing done status to $status for the case with the ID of $case_id.";
