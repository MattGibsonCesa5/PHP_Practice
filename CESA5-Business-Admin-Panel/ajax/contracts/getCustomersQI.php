<?php 
    ...
    if (isset($_SESSION["status"]) && $_SESSION["status"] == 1)
    {
        ...
        if (checkUserPermission($conn, "BUILD_QUARTERLY_INVOICES"))
        {
            ...
            { 
                ...
                $temp["isBuild"] = $customer["build_quarterly_invoice"];
                
                // build the folders column
                $folders_display = "";
                if ($customer["invoice_folder_id"] == null || $customer["invoice_folder_id"] == "") { $display_sc_folder = "<span class=\"missing-field\">No folder assigned</span>"; }
                else { $display_sc_folder = $customer["invoice_folder_id"]; }
                ...
                $actions = "";
                $build_contract = $customer["build_quarterly_invoice"];
                if ($build_contract == 1) { $actions .= "<button class='btn btn-success w-100 mb-1' id='QI-$customer_id' value='1' onclick='toggleBuild(\"QI\", $customer_id);'><i class='fa-solid fa-check'></i></button>"; }
                else { $actions .= "<button class='btn btn-danger w-100 mb-1' id='QI-$customer_id' value='0' onclick='toggleBuild(\"QI\", $customer_id);'><i class='fa-solid fa-xmark'></i></button>"; }
                $actions .= "<button class='btn btn-primary w-100 mb-1' type='button' onclick='getBuildModal(\"QI\", $customer_id);'><i class='fa-solid fa-hammer'></i></button>"; // build button
